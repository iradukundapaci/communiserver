...
import { createIsiboLeader } from "@/lib/api/leaders";
...
export default function CreateIsiboLeaderPage() {
  ...
  const fetchIsibos = async () => {
    ...
    try {
      ...
      const response = await getIsibos(formData.villageId, 1, 100); // Get all isibos for the selected village
  ...
  return (
    <PermissionRoute permission={Permission.CREATE_ISIBO_LEADER}>
      <div className="flex flex-col gap-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-3xl font-bold">Create Isibo Leader</h1>
        </div>

        <Card>
          <form onSubmit={handleSubmit}>
            <CardHeader>
              <CardTitle>Isibo Leader Information</CardTitle>
              <CardDescription>
                Enter the details for the new isibo leader
              </CardDescription>
            </CardHeader>
            ...
            <CardFooter className="flex justify-end space-x-2">
              ...
              <Button type="submit" disabled={isLoading}>
                {isLoading ? "Creating..." : "Create Isibo Leader"}
              </Button>
            </CardFooter>
          </form>
        </Card>
